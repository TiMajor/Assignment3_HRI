...
val AskQuestion: State = state(parent = Parent) {
    ...
    onResponse<AnswerOption> {
        ...
        if (answer.correct) {
            ...
            furhat.say("Sorry, that was ${furhat.voice.emphasis("not")} correct. The correct answer is ${QuestionSet.current.getCorrectAnswer()}")
    ...
    onResponse<DontKnow> {
        furhat.say("Don't worry. The correct answer is ${QuestionSet.current.getCorrectAnswer()}. Here comes the next question")
        goto(NewQuestion)
    }
    ...
    onNoResponse {
        random(
                { furhat.say("Sorry, I cannot hear you") },
                { furhat.say("So, what is your answer?") }
        )
        goto(NewQuestion)
    }
